...
if current_round == 1:
    train_idx = np.random.choice(all_idx, int(len(train_dataset)/node_count),
                                 replace=False)
    np.savetxt('train/iid/fmnist_'+str(node_id), train_idx)
else:
    train_idx = np.loadtxt('train/iid/fmnist_'+str(node_id))
...
if resume and current_round != 1:
    path_checkpoint = "train/iid/checkpoint/agg/fmnist_ckpt_" + str(current_round-1) + ".pth"  # 断点路径
...
def train(epoch):
    ...
    if not os.path.isdir("train/iid/checkpoint/fmnist/"):
        os.mkdir("train/iid/checkpoint/fmnist/")
    torch.save(checkpoint, 'train/iid/checkpoint/fmnist/ckpt_'+ str(node_id) + '_%s.pth' %(str(epoch)))
