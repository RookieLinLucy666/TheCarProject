...
const (
	Address = "WcoSdfXTvDFuXHooXXeNh6Vt98zp56b9i"
	Contract_Addr = "XC1111111111111112@xuper"
	Contract_Name = "adv14s"
	Mnemonic = "哪 甜 脉 符 混 会 瞧 凶 拥 扫 衰 取"
)
...
//  @Description: 创建账户
//	命令行转钱：
//	./xchain-cli transfer --to WcoSdfXTvDFuXHooXXeNh6Vt98zp56b9i --amount 100000000 --keys data/keys/ -H 127.0.0.1:37101
//**/
//func CreateAccount() {
...
//  @Description: 创建合约账户
//	命令行给合约账户转钱：
//	./xchain-cli transfer --to XC1111111111111112@xuper --amount 1000000000000
//**/
...
**/
func DeployContract() {
	codePath := "contract/adv.wasm" // 编译好的二进制文件 go build -o counter
	...
	tx, err = xchainClient.DeployWasmContract(account, contractName, code, args)
	if err != nil {
		panic(err)
	}
	fmt.Printf("Deploy Native Contract Success! %x\n", tx.Tx.Txid)
}

func InvokeCreateAdv(uploader string, name string, argtype string, ip string, route string, abstract string) string{
	account := getAccount()
	...
	var tx *xuper.Transaction
	tx, err = xchainClient.InvokeWasmContract(account, contractName, "createAdv", args)
	if err != nil {
		panic(err)
	}
	if tx != nil {
		fmt.Println("InvokeCreateAdv: ", string(tx.ContractResponse.Body))
	}
...
func InvokeQuery(id string) {
	...
	var tx *xuper.Transaction
	tx, err = xchainClient.InvokeWasmContract(account, contractName, "query", args)
	if err != nil {
		panic(err)
	}
	if tx != nil {
		fmt.Println("InvokeQuery: ", string(tx.ContractResponse.Body))
	}
}

func InvokeQueryCallback(id string, data string) {
	account := getAccount()
	...
	var tx *xuper.Transaction
	tx, err = xchainClient.InvokeWasmContract(account, contractName, "queryCallback", args)
	if err != nil {
		panic(err)
	}
	if tx != nil {
		fmt.Println("InvokeQueryCallback: ", string(tx.ContractResponse.Body))
	}
}
...
func InvokeComputingShare(id string, model string, dataset string, round string, epoch string) {
	...
	tx, err = xchainClient.InvokeWasmContract(account, contractName, "computingshare", args)
	if err != nil {
		panic(err)
	}
	if tx != nil {
		fmt.Println("InvokeComputingShare: ", string(tx.ContractResponse.Body))
	}
}

func InvokeComputingShareCallback(id string, data string) {
	account := getAccount()
	...
	tx, err = xchainClient.InvokeWasmContract(account, contractName, "computingCallback", args)
	if err != nil {
		panic(err)
	}
	if tx != nil {
		fmt.Println("InvokeComputingCallBack: ", string(tx.ContractResponse.Body))
	}
}
...
func InvokeAddUser(name string, abstract string) string{
	...
	var tx *xuper.Transaction
	tx, err = xchainClient.InvokeWasmContract(account, contractName, "addUser", args)
	if err != nil {
		panic(err)
	}
	if tx != nil {
		fmt.Println("InvokeAddUser: ", string(tx.ContractResponse.Body))
	}
	return string(tx.ContractResponse.Body)
}

func InvokeVerifyUser (name string, abstract string) string{
	account := getAccount()
	...
	var tx *xuper.Transaction
	tx, err = xchainClient.InvokeWasmContract(account, contractName, "verifyUser", args)
	if err != nil {
		panic(err)
	}
	if tx != nil {
		fmt.Println("InvokeVerifyUser: ", string(tx.ContractResponse.Body))
	}
	return string(tx.ContractResponse.Body)
}
...
func abstractComputingEvent(event string) (Metadata, FederatedAIDemand){
	...
	if err != nil {
		fmt.Printf("Error decoding string:  ", err.Error())
...
func Run() {
	DeployContract()
	id := InvokeCreateAdv("xuperchain", "counter", "data", "127.0.0.1", "xuperchain","162accb12e079d4b805f65f7a773c5e10cf537fef5ff99fde901ef0b1c963af8")
	InvokeQuery(id)
	InvokeQueryCallback(id, "91.2")
	InvokeComputingShare(id, "cnn", "mnist", "1", "1")
	InvokeComputingShareCallback(id, "91.2")
	InvokeAddUser("Alice", "WcoSdfXTvDFuXHooXXeNh6Vt98zp56b9i")
	InvokeVerifyUser("Alice", "WcoSdfXTvDFuXHooXXeNh6Vt98zp56b9i")
	InvokeVerifyUser("Alice", "WcoSdfXTvDFuXHooXXeNh6Vt98zp56b9s")
}
